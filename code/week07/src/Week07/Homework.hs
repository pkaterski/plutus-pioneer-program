...
--                | Draw
-- 
-- gameCompare :: GameChoice -> GameChoice -> GameCheck
-- gameCompare Rock Rock         = Draw
-- gameCompare Rock Scissors     = Beat
-- gameCompare Rock Paper        = Loose
-- gameCompare Paper Rock        = Beat
-- gameCompare Paper Scissors    = Loose
-- gameCompare Paper Paper       = Draw
-- gameCompare Scissors Rock     = Loose
-- gameCompare Scissors Scissors = Draw
-- gameCompare Scissors Paper    = Beat
...
check :: BuiltinByteString
      ...
      -> ScriptContext
      -> Bool
check bsRock' _        _           (GameDatum bs (Just c)) (Reveal nonce Rock)     _ =
  sha2_256 (nonce `appendByteString` bsRock')     == bs && (beats Rock c || c == Rock)
check _       bsPaper' _           (GameDatum bs (Just c)) (Reveal nonce Paper)    _ =
  sha2_256 (nonce `appendByteString` bsPaper')    == bs && (beats Paper c || c == Paper)
check _       _        bsScissors' (GameDatum bs (Just c)) (Reveal nonce Scissors) _ =
  sha2_256 (nonce `appendByteString` bsScissors') == bs && (beats Scissors c || c == Scissors)
...
gameChoiseToBs :: GameChoice -> BuiltinByteString
gameChoiseToBs Rock     = bsRock
gameChoiseToBs Paper    = bsPaper
gameChoiseToBs Scissors = bsScissors
...
firstGame fp = do
    ...
    let game   = Game
        ...
        c      = fpChoice fp
        bs     = sha2_256 $ fpNonce fp `appendByteString` (gameChoiseToBs c)
    ...
    case m of
        ...
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            ...
            GameDatum _ (Just c') | c' == c || c `beats` c' -> do
                logInfo @String $ "second player played and lost"-- ++ show (fpChoice fp)
