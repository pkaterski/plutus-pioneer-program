...
tsStateMachine ts = mkStateMachine (Just $ tsTT ts) (transition ts) (== Nothing) -- for final state?
...
closeTS ts = void $ mapErrorSM $ runStep (tsClient ts) $ Close
