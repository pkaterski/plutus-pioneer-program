...
mkPolicy pkh deadline () ctx = traceIfFalse "missing signature" signed &&
                               traceIfFalse "deadline passed"   deadlinePassed
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    signed :: Bool
    signed = txSignedBy info $ unPaymentPubKeyHash pkh

    deadlinePassed :: Bool
    deadlinePassed = to deadline `contains` txInfoValidRange info
